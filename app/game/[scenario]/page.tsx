...
// ⚡ Bolt Optimization: Extract message map body into a separate component wrapped in React.memo
// This prevents older messages from re-rendering on every streamed token when the parent component updates its streaming state.
// Expected performance impact: O(N) re-renders of the entire message history are eliminated during LLM streaming.
const MessageItem = memo(function MessageItem({
  ...
  return (
    <ChatBubble
      content={m.content}
      isUser={m.role === 'user'}
      avatar={m.role === 'assistant' ? npcAvatar : undefined}
      wisdomIcon={m.wisdomCard ? '🦉' : undefined}
      onWisdomClick={m.wisdomCard ? () => onShowWisdomPopup(m.wisdomCard!) : undefined}
    >
      ...
      {isObserve && m.role === 'assistant' && (m.npcThoughts || m.psychologyNote) && (
        <PsychologyNote
          ...
          isExpanded={isExpandedNote}
          onToggle={() => onTogglePsychology(m.id)}
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-2">
        ...
        {messages.map(m => (
          <MessageItem
            ...
            lang={lang}
            isExpandedNote={expandedNotes.has(m.id)}
            isExpandedCultural={expandedCultural.has(m.id)}
