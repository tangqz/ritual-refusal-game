...
/**
 * ⚡ Bolt Optimization: Extracted message rendering into a memoized component.
 *    By using React.memo and ensuring props are primitive or stable references,
 *    we prevent all previous messages from re-rendering on every single character
 *    token streamed by the LLM.
 */
const MessageItem = React.memo(function MessageItem({
  ...
  onShowWisdomPopup: (wisdom: AuntieWisdom) => void;
}) {
  if (m.role === 'context') {
  ...
  const handleWisdomClick = m.wisdomCard ? () => onShowWisdomPopup(m.wisdomCard) : undefined;
  const handleTogglePsychology = () => onTogglePsychology(m.id);
  const handleToggleCultural = () => onToggleCultural(m.id);
