...
export async function POST(request: NextRequest) {
  ...
  // SECURITY: Prioritize request.ip to prevent IP spoofing via x-forwarded-for headers
  const ip = (request as any).ip || request.headers.get('x-forwarded-for')?.split(',')[0].trim() || request.headers.get('x-real-ip') || 'anonymous';
